...
def publishEvent(eventInfo: EventInfo, config: Config) -> Result:
    ...
    try:
        ...
        if len(gCalConflicts) > 0 and not config.ignoreResolveableConflicts:
            logging.error(
                "EventPublisher: Found gCal conflicts %s ", str(gCalConflicts)
            )
...
if __name__ == "__main__":
    ...
    zoomConfig = ZoomAPI.ZoomConfig(accountId="", clientId="", clientSecret="")
    anConfig = ActionNetworkAutomation.ANAutomatorConfig(email="", password="")
    gCalConfig = GoogleCalendarAPI.GoogleCalendarConfig(
        serviceKeyPath="", calendarId="", delegateAccount=""
    )
    result = publishEvent(
        eventInfo=event,
        config=Config(zoomConfig=zoomConfig, anConfig=anConfig, gCalConfig=gCalConfig),
    )
    print(result)
