...
def main(args):
    ...
    if flags.archive:
        ...
        ReccommitUtils.writeCSVFile(os.path.join(Constants.ARCHIVE_FOLDER_PATH, archiveName), newCols, archiveRows)
    # Retention
    if flags.retention:
        print("Retention")
        ...
        for row in rows:
            ...
            if status == Constants.MEMBERSHIP_STATUS.GOOD_STANDING:
                ...
                print(status)
                return
        ReccommitUtils.appendCSVFile(Constants.RETENTION_DATA_FILE_PATH, (Constants.TODAY_STR, membersGoodStanding, membersMember, membersLapsed, membersGoodStanding+membersMember+membersLapsed))
    ...
    if flags.actionNetwork:
        ...
        ReccommitUtils.writeCSVFile(os.path.join(Constants.OUTPUT_DIR_PATH, "action-network-"+Constants.TODAY_STR+".csv"), newCols, actionNetworkRows)
    else:
    ...
    alreadyCalledListCols, alreadCalledListRows = ReccommitUtils.readCSV(ReccommitUtils.Constants.Paths.RECOMMIT_ALREADY_CALLED_FULL)
    alreadyCalledListColsIndexes = ReccommitUtils.getIndexesForColumns(alreadyCalledListCols)
    
    doNotCallCols, doNotCallRows = ReccommitUtils.readCSV(ReccommitUtils.Constants.Paths.RECOMMIT_DNC)
    doNotCallColsIndexes = ReccommitUtils.getIndexesForColumns(doNotCallCols)

    alreadyCalledAndDNCEmailSet = set([x[alreadyCalledListColsIndexes[ReccommitUtils.Constants.Columns.EMAIL]].strip().lower() for x in alreadCalledListRows+doNotCallCols])

    recommitRows = []
    recommitCols = ReccommitUtils.Constants.RECOMMIT_COLS_FOR_NEW_LIST
    colIndexes = [firstNameColIndex, lastNameColIndex, standingIndex, emailIndex, phoneNumberIndex]
    for row in rows:
        if len(recommitRows) >= Constants.RECOMMIT_LIST_SIZE:
            break
        if row[standingIndex] == Constants.MEMBERSHIP_STATUS.MEMBER and row[emailIndex].strip().lower() not in alreadyCalledAndDNCEmailSet and row[phoneNumberIndex].strip() != "":
            recommitRows.append([row[i] for i in colIndexes])
    if len(recommitRows) < Constants.RECOMMIT_LIST_SIZE:
        for row in rows:
            if len(recommitRows) >= Constants.RECOMMIT_LIST_SIZE:
                break
            if row[standingIndex] == Constants.MEMBERSHIP_STATUS.LAPSED and row[emailIndex].strip().lower() not in alreadyCalledAndDNCEmailSet and row[phoneNumberIndex].strip() != "":
                recommitRows.append([row[i] for i in colIndexes])
    ReccommitUtils.writeCSVFile(os.path.join(Constants.RECOMMIT_DIR, "nextBankList.csv"), recommitCols, recommitRows)
