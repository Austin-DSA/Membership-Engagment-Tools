...
def main(args):
    ...
    #Read in list
    print("This tool is meant to generate the copy paste into the shared tracking list, it will not accumulate from previous runs.")
    print("It is expected that in the same directory as this script there is a 'recommit' directory, all output will be written there.")
    print("Any Existing Files in the recommit files may be overwritten. Press any button to confirm. Ctrl-C or exit terminal to exit.")
    input("Continue?")
    ...
    alreadyCalledCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
    ...
    def transformRowToAlreadyCalledRow(colIndexes, row):
        newRow =  [row[colIndexes[c]] for c in alreadyCalledCols if c != ReccommitUtils.Constants.Columns.DATE_CALLED]
        newRow.append(commandArgs.date)
        return newRow

    movedCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
                 ReccommitUtils.Constants.Columns.LAST_NAME, 
                 ReccommitUtils.Constants.Columns.EMAIL]
    movedRows = []
    ...
    dncCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
               ReccommitUtils.Constants.Columns.LAST_NAME, 
               ReccommitUtils.Constants.Columns.EMAIL]
    dncRows = []
    ...
    wrongNumberCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
                         ReccommitUtils.Constants.Columns.LAST_NAME, 
                         ReccommitUtils.Constants.Columns.EMAIL]
    wrongNumberRows = []
    ...
    needsFollowupCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
                         ReccommitUtils.Constants.Columns.LAST_NAME, 
                         ReccommitUtils.Constants.Columns.EMAIL,
                         ReccommitUtils.Constants.Columns.CALLER,
                         ReccommitUtils.Constants.Columns.DATE_CALLED]
    ...
    def transformRowToNeedsFollowupRow(colIndexes, row):
        newRow = [row[colIndexes[c]] for c in needsFollowupCols if c != ReccommitUtils.Constants.Columns.DATE_CALLED]
        newRow.append(commandArgs.date)
        return newRow
    
    doesNotWantToBeAMemberCols = [ReccommitUtils.Constants.Columns.FIRST_NAME, 
                         ReccommitUtils.Constants.Columns.LAST_NAME, 
                         ReccommitUtils.Constants.Columns.EMAIL]
    ...
    for callRow in callListRows:
        response = callRow[callListColsIndexes[ReccommitUtils.Constants.Columns.RESPONSE]].strip().lower()
        if response == ReccommitUtils.Constants.Responses.MOVED.lower():
            ...
            alreadyCalledRows.append(transformRowToAlreadyCalledRow(callListColsIndexes, callRow))
            ...
            wrongNumberRows.append(transformRowToWrongNumberRow(callListColsIndexes, callRow))
            alreadyCalledRows.append(transformRowToAlreadyCalledRow(callListColsIndexes, callRow))

        elif response == ReccommitUtils.Constants.Responses.NO_MEMBER.lower():
            doesNotWantToBeAMemberRows.append(transformRowToDoesNotWantToBeAMemberRow(callListColsIndexes, callRow))
            alreadyCalledRows.append(transformRowToAlreadyCalledRow(callListColsIndexes, callRow))
            ...
            needsFollowupRows.append(transformRowToNeedsFollowupRow(callListColsIndexes, callRow))
            alreadyCalledRows.append(transformRowToAlreadyCalledRow(callListColsIndexes, callRow))
    ...
    # Write outputs
    ReccommitUtils.writeCSVFile(ReccommitUtils.Constants.Paths.RECOMMIT_ALREADY_CALLED, alreadyCalledCols, alreadyCalledRows)
    ...
    ReccommitUtils.writeCSVFile(ReccommitUtils.Constants.Paths.RECOMMIT_TO_RESOLVE, consolidatedCols, consolidatedRows)
    print("Done")
