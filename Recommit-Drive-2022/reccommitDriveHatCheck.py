...
def calculateConfirmedRecommit(
    callTrackingPath, membershipPath
    ...
    membersInGoodStanding = set(
        ReccommitUtils.getListOfEmailsInGoodStanding(membershipCols, membershipRows)
    )
    colIndexes = ReccommitUtils.getIndexesForColumns(
        callTrackingCols,
        [
            ReccommitUtils.Constants.CALL_TRACKING_COLS.CALLER,
            ReccommitUtils.Constants.CALL_TRACKING_COLS.EMAIL,
        ],
    )
    ...
    for row in callTrackingRows:
        ...
        caller = (
            row[colIndexes[ReccommitUtils.Constants.CALL_TRACKING_COLS.CALLER]]
            .strip()
            .lower()
        )
        member = (
            row[colIndexes[ReccommitUtils.Constants.CALL_TRACKING_COLS.EMAIL]]
            .strip()
            .lower()
        )
        numConfirmed = 0
...
def main(args: list[str]):
    ...
    outputCols = [
        "Email",
        "Phone Bank Attendance",
        "Poltical Education Attendance",
        "Confirmed Recommitted",
        "Calls Made",
    ]
    outputRows = []
    ...
    callTrackingMap = calculateConfirmedRecommit(
        commandArgs.callTrackingPath, commandArgs.membershipPath
    )
    ...
    ReccommitUtils.writeCSVFile(
        ReccommitUtils.Constants.Paths.RECOMMIT_HAT_CHECK_OUTPUT, outputCols, outputRows
    )
