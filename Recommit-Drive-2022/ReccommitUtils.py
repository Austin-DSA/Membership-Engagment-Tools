...
class Constants:
    ...
    class Paths:
        ...
        RECOMMIT_ALREADY_CALLED_FULL = os.path.join(
            RECOMMIT_DIR, "alreadyCalledFull.csv"
        )
...
def getIndexesForColumns(cols):
    ...
    for i in range(len(cols)):
        if cols[i] not in Constants.KNOWN_COLUMNS and cols[i] != "":
            print(
                "Error: When parsing columns for indexes, found unkown column: "
                + str(cols[i])
            )
            print("Exiting")
            sys.exit(1)
...
def countAttendanceFromCondensedList(rows, listPrefix, emailCol) -> dict[str, int]:
    ...
    while rowIndex < len(rows):
        match state:
            ...
            case _:
                raise ReccommitException(
                    "Unexpected State:"
                    + state
                    + " when counting attendance for "
                    + str(rows)
                )
    return output
...
def countPhoneBankAttendance(phoneBankAttendancePath) -> dict[str, int]:
    ...
    rows.insert(0, cols)
    return countAttendanceFromCondensedList(
        rows, Constants.PHONE_BANK_COLS.PREFIX, Constants.PHONE_BANK_COLS.EMAIL
    )


def countPoliticalEducationAttendance(
    politicalEducationAttendancePath,
) -> dict[str, int]:
    cols, rows = readCSV(politicalEducationAttendancePath)
    rows.insert(0, cols)
    return countAttendanceFromCondensedList(
        rows, Constants.POLI_ED_COLS.PREFIX, Constants.POLI_ED_COLS.EMAIL
    )
    ...
    for col in wantedCols:
        if col not in colToIndexMap:
            raise UtilsException(
                "Error: Couldn't find column " + str(col) + " in cols " + str(fullCols)
            )
    return colToIndexMap
...
def getListOfEmailsInGoodStandingFromMembershipList(membershipListPath) -> list[str]:
    ...
    colToIndexMap = getIndexesForColumns(
        cols,
        [
            Constants.MEMBERSHIP_LIST_COLS.EMAIL_COL,
            Constants.MEMBERSHIP_LIST_COLS.STANDING_COL,
        ],
    )
    return getListOfEmailsInGoodStandingWithIndex(
        rows=rows,
        statusIndex=colToIndexMap[Constants.MEMBERSHIP_LIST_COLS.STANDING_COL],
        emailIndex=colToIndexMap[Constants.MEMBERSHIP_LIST_COLS.EMAIL_COL],
    )
...
def getListOfEmailsInGoodStandingWithIndex(
    rows: list[list[str]], statusIndex: int, emailIndex: int
) -> list[str]:
