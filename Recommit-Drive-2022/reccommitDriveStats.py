...
class CommmandFlags:
    ...
    def __init__(
        ...
        membershipPath,
        callTrackingPath,
        phoneBankAttendancePath,
        politicalEducationAttendancePath,
    ) -> None:
...
def parseArgs(args: list[str]) -> CommmandFlags:
    if CommmandFlags.HELP in args:
        print(
            "usage: <current membership list> <call tracking list path> <phone bank attendance list> <political education attendance list>\n Will output to files(overwrite if they already exist) in reccomit/final-stats directory"
        )
        sys.exit(0)
    if len(args) != 5:
    ...
    return CommmandFlags(
        ...
        callTrackingPath=callTrackingPath,
        phoneBankAttendancePath=phoneBankAttendancePath,
        politicalEducationAttendancePath=politicalEducationAttendancePath,
    )
...
def main(args: list[str]):
    ...
    outputRows = [
        ["Reccommit Stats Generated:", datetime.datetime.now().strftime("%Y-%m-%d")]
    ]
    ...
    phoneBankAttendanceMap = ReccommitUtils.countPhoneBankAttendance(
        commandArgs.phoneBankAttendancePath
    )
    politicalEdAttendanceMap = ReccommitUtils.countPoliticalEducationAttendance(
        commandArgs.politicalEducationAttendancePath
    )
    attendeesOfBoth = set.intersection(
        set(phoneBankAttendanceMap.keys()), set(politicalEdAttendanceMap.keys())
    )
    outputRows.append([])
    outputRows.append(["Attendance Stats:"])
    outputRows.append(
        [
            "Unique Political Education Attendees:",
            str(len(politicalEdAttendanceMap.keys())),
        ]
    )
    outputRows.append(
        ["Unique Phone Bank Attendees:", str(len(phoneBankAttendanceMap.keys()))]
    )
    ...
    memberEmailsInGoodStanding = (
        ReccommitUtils.getListOfEmailsInGoodStandingFromMembershipList(
            commandArgs.membershipPath
        )
    )
    for row in trackingRows:
        numCalls += 1
        response = (
            row[trackingColsIndexes[ReccommitUtils.Constants.Columns.RESPONSE]]
            .strip()
            .lower()
        )
        email = (
            row[trackingColsIndexes[ReccommitUtils.Constants.Columns.EMAIL]]
            .strip()
            .lower()
        )
    ...
    outputRows.append(
        [
            "Response",
            "Number of Calls",
            "Number of Confirmed Reccommit",
            "Reccommit Percentage",
        ]
    )
    outputRows.append(
        [
            ReccommitUtils.Constants.Responses.DNC,
            numDoNotCall,
            numDoNotCallConfirmed,
            numDoNotCallConfirmed / numDoNotCall,
        ]
    )
    outputRows.append(
        [
            ReccommitUtils.Constants.Responses.NO_MEMBER,
            numNoMember,
            numNoMemberConfirmed,
            numNoMemberConfirmed / numNoMember,
        ]
    )
    outputRows.append(
        [
            ReccommitUtils.Constants.Responses.MOVED,
            numMoved,
            numMovedConfirmed,
            numMovedConfirmed / numMoved,
        ]
    )
    outputRows.append(
        [
            ReccommitUtils.Constants.Responses.RECOMMITTED,
            numReccommit,
            numReccommitConfirmed,
            numReccommitConfirmed / numReccommit,
        ]
    )
    outputRows.append(
        [
            ReccommitUtils.Constants.Responses.CONTACTED,
            numContacted,
            numContactedConfirmed,
            numContactedConfirmed / numContacted,
        ]
    )
    outputRows.append(
        ["All Calls", numCalls, numCallsConfirmed, numCallsConfirmed / numCalls]
    )

    ReccommitUtils.writeCSVFile(
        ReccommitUtils.Constants.Paths.RECOMMIT_STATS_OUTPUT,
        outputRows[0],
        outputRows[1:],
    )
